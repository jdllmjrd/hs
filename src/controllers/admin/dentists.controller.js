...
exports.updateDentist = (req, res, next) => {
  ...
  featuredDentist
    .update(
      ...
      { include: ["admin_updated_dentist"] }
    )
    .then((result) => {
      console.log(req.body);
      if (result) {
        // retrieve updated details
        Users.findByPk(dentists_id).then((result) => {
          res.send({
            error: false,
            data: result,
  ...
  featuredDentist
    ...
    .then((data) =>
      dataResponse(
        res,
        data,
        "Featured Dentist Retrieved Successfully",
...
exports.deleteDentist = (req, res) => {
  ...
  featuredDentist
    ...
    .then((result) => {
      if (result)
        emptyDataResponse(res, "Featured is successfully deactivated");
    })
    .catch((err) => errResponse(res, err));
};
